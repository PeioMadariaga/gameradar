...
def vectorize(batch):
    ...
    nums = np.stack(
        [batch["price_eur"], batch["marketing_budget_k"]],
        axis=1
    ).astype(np.float32)
    bools = np.stack(
        [batch["is_sequel"], batch["has_crossplay"], batch["coop"]],
        axis=1
    ).astype(np.float32)
    return np.concatenate([Xg, Xp, nums, bools], axis=1)

def gen_synth(n=3000, rng=np.random.default_rng(7)):
    # Categóricas multi-etiqueta
    g = np.array(
        [rng.choice(GENRES, size=rng.integers(1, 3), replace=False).tolist()
         for _ in range(n)],
        dtype=object
    )
    p = np.array(
        [rng.choice(PLATFORMS, size=rng.integers(1, 3), replace=False).tolist()
         for _ in range(n)],
        dtype=object
    )

    # Numéricas / booleanas
    price  = rng.uniform(50, 99, size=n).astype(np.float32)     # rango realista
    ...
    # ---------------- Tratamiento de PRECIO (óptimo ~70–80 €) ----------------
    # Óptimo real: el éxito sube hacia 70–80 y cae si te alejas (por barato o por caro)
    ideal_low, ideal_high = 70.0, 80.0

    # Penalización en U (cuadrática) fuera del intervalo óptimo
    ...
    # Tolerancia premium: secuela/cross/coop reducen penalización por caro (nunca aumenta)
    tol = 1.0 - 0.25*(0.4*cross + 0.3*coop + 0.3*sequel)
    price_penalty *= np.clip(tol, 0.7, 1.0)

    # Marketing compensa un poco (y empuje general)
    mk_boost = 0.0004 * (budget - 120.0)
...
def main():
    ...
    with open(os.path.join(artifacts, "meta.json"), "w") as f:
        json.dump(
            {"GENRES": GENRES, "PLATFORMS": PLATFORMS, "COUNTRIES": COUNTRIES},
            f
        )
